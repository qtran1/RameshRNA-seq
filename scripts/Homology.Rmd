...
overlap_ecc1_t47d <- intersect(homolog_ecc1_esr1$Associated.Gene.Name, homolog_t47d_esr1$Associated.Gene.Name)
...
write.csv(as.data.frame(overlap_of_overlap), file="../data/processed/Overlap_ERB_and_ESR1_in_T47D_and_ECC1.csv")
write.csv(as.data.frame(overlap_ecc1_t47d), file="../data/processed/Overlap_ESR1_T47D_and_ECC1.csv")
...
gene_list_ecc1_t47d <- list(homolog_ecc1_esr1$Associated.Gene.Name, homolog_t47d_esr1$Associated.Gene.Name)
gene_3way_overlap <- list(wat_genes,homolog_ecc1_esr1$Associated.Gene.Name,homolog_t47d_esr1$Associated.Gene.Name)

library(Vennerable)
pdf("figures/Venn_RNAseq-ChIPseq-ECC1.pdf")
v.ecc1 <- Venn(gene_list_ecc1, SetNames=c("ER-beta-RNAseq", "ESR1-ChIPseq-ECC1"))
plot(v.ecc1)
dev.off()

pdf("figures/Venn_RNAseq-ChIPseq-T47D.pdf")
v.t47d <- Venn(gene_list_t47d, SetNames=c("ER-beta-RNAseq", "ESR1-ChIPseq-T47D"))
plot(v.t47d)
dev.off()

pdf("figures/Venn_ERbeta-ERalpha-ECC1-T47D.pdf")
v.3way <- Venn(gene_3way_overlap, SetNames=c("ER-beta-RNAseq", "ESR1-ChIPseq-ECC1", "ESR1-ChIPseq-T47D"))
plot(v.3way)
dev.off()
