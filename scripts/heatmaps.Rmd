...
plot(regHeatmap(all.sig.matrix, legend=2, col=heat.colors, breaks=-3:3))
corrdist = function(x) as.dist(1-cor(t(x)))
hclust.avl = function(x) hclust(x, method="average")
reg3 <- regHeatmap(all.sig.matrix, legend=2, dendrogram=list(clustfun=hclust.avl, distfun=corrdist), 
                   col=heatmap.colors, breaks=-4:4)
map <- annHeatmap(all.sig.matrix, ann=list(Col=list(data=ordered.mapping.data)), col=g2r.colors)
plot(map)
...
map <- annHeatmap(sig.matrix, ann=list(Col=list(data=liver.mapping.data)), col=g2r.colors)
plot(map)
```
...
diet.map <- annHeatmap(diet.sig.matrix, ann=list(Col=list(data=liver.mapping.data)), col=g2r.colors)
plot(diet.map)
```
...
wat.map <- annHeatmap(wat.sig.matrix, ann=list(Col=list(data=wat.mapping.data)), col=g2r.colors)
plot(wat.map)
```
...
wat.map <- annHeatmap(wat.sig.diet, ann=list(Col=list(data=wat.mapping.data)), col=g2r.colors)
plot(wat.map)
```
